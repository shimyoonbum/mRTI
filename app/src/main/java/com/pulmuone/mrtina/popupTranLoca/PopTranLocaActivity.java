...
public class PopTranLocaActivity extends Activity{

    private PopTranLocaAdapter locaAdapter;
    ArrayList<PopTranLocaItem> locaList = null;
    ...
    OnClickListener onClickListener = new OnClickListener() {
        ...
        public void onClick(View arg0) {
            if (arg0.equals(btn_close)){
                ...
                param.put("searchLocator", Util.nullString(locator,""));

                PopTranLocaActivity.network = new Network(PopTranLocaActivity.this, listener, "/api/tran/tranLocatorList");
                PopTranLocaActivity.network.execute(param);
            }
        }
    };
    ...
    NetworkEvent listener = new NetworkEvent() {
        ...
        public void onPostExecute(JSONObject obj, String api) {
            ...
            if(api.equals("/api/tran/tranLocatorList")) {
                try {
                    ...
                    for(int i=0; i<list.length(); i++){
                        ...
                        PopTranLocaItem info = new PopTranLocaItem(Util.nullString(data.getString("INVENTORY_LOCATION_ID"),"")
                                , Util.nullString(data.getString("LOCATOR_CODE"),"")
                                , Util.nullString(data.getString("LOCATOR_NAME"),"")
                        );
                        locaList.add(info);
                    }
                    locaAdapter = new PopTranLocaAdapter(PopTranLocaActivity.this, locaList);
                    ...
                    list_loca.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                        ...
                        public void onItemClick(AdapterView<?> parent, View view, final int position, long id) {
                            ...
                            PopTranLocaItem item = (PopTranLocaItem) locaAdapter.getItem(position);
    ...
    private void checkList(JSONArray list) {
        if(list.length() == 0){
            AlertDialog.Builder builder = new AlertDialog.Builder(PopTranLocaActivity.this);
            builder
